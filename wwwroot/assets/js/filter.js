function filterCategory() {
    $(document).on('click', '.category-filter', async function (e) {
        ...
        var categoryValue = $(this).attr('category-id');

        await fetch('/shop/filterCategory?filterId=' + categoryValue)
            .then(response => {
                if (!response.ok) {
...
function filterBrand() {
    $(document).on('click', '.brand-filter', async function (e) {

        let datas = []

        e.preventDefault();

        var brandValue = $(this).attr('brand-id');

        await fetch('/shop/filterBrand?filterId=' + brandValue)
            .then(response => {
                if (!response.ok) {
                    // Response not OK, throw an error with status text
                    return response.text().then(text => {
...
function filterColor() {
    $(document).on('click', '.color-filter', async function (e) {

        let datas = []

        e.preventDefault();

        var brandValue = $(this).attr('color-id');

        console.log(brandValue)
        await fetch('/shop/filterColor?filterId=' + brandValue)
            .then(response => {
                if (!response.ok) {
                    // Response not OK, throw an error with status text
                    return response.text().then(text => {
